  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? "bg-[#09090b]/96 backdrop-blur-xl border-b border-white/[0.06]"
          : "bg-transparent"
      }`}
    >
      <div className="max-w-[1320px] mx-auto px-6 lg:px-10 flex items-center justify-between h-[68px]">
        ...
        <div className="hidden md:flex items-center gap-2.5">
          <Link
            to="/signin"
            className="text-[13px] text-[#94a3b8] hover:text-white transition-colors duration-200 font-body font-[450] px-3 py-2"
          >
            Sign in
          </Link>
          <Link
        ...
        <button
          ...
          className="md:hidden text-[#cbd5e1] hover:text-white transition-colors p-2 -mr-1.5 min-h-[44px] min-w-[44px] flex items-center justify-center"
      ...
      {/* Mobile drawer */}
      <div id="marketing-menu" inert={!open} className={`md:hidden transition-all duration-300 overflow-hidden ${open ? "max-h-96" : "max-h-0"}`}>
        <div className="bg-[#18181b] border-t border-white/[0.06] px-6 py-5 space-y-1">
          {links.map((l) => (
            <NavLink
              key={l.to}
              to={l.to}
              ...
              className={({ isActive }) =>
                `flex items-center text-[15px] font-body font-[450] py-2.5 px-3 rounded-md transition-colors min-h-[44px] ${
                  isActive ? "text-brand-lime bg-brand-lime/5" : "text-[#cbd5e1] hover:text-white hover:bg-white/[0.04]"
                }`
              }
            >
              {l.label}
