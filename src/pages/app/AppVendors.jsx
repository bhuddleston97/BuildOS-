...
const complianceCfg = {
  ...
  pending:       { label: "Pending Review", color: "text-[#8a9b8e]",   bg: "bg-white/[0.04]",   border: "border-white/[0.08]",  Icon: Shield },
};
...
export default function AppVendors() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      listRows("vendors", { perPage: 100, sort: "name", signal: ctrl.signal }),
      listRows("subcontractors", { perPage: 100, sort: "name", signal: ctrl.signal }),
    ]).then(([v, s]) => { setVendors(v); setSubs(s); })
    ...
    <div className="p-6 lg:p-8 max-w-[1200px] mx-auto">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-[14px] text-[#6b7a6e] font-body mt-0.5">{vendors.length} vendor{vendors.length!==1?"s":""} · {subs.length} subcontractor{subs.length!==1?"s":""}</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {Object.entries(complianceCfg).map(([k, c]) => {
          ...
          return (
            <div key={k} className={`bg-[#0f1410] border rounded-xl p-4 ${c.border}`}>
      ...
      <div className="flex bg-white/[0.04] border border-white/[0.08] rounded-xl p-1 mb-5 w-fit">
        {[["vendors","Vendors"],["subs","Subcontractors"]].map(([k,label])=>(
          <button key={k} onClick={() => setTab(k)}
            className={`px-4 py-2 text-[13px] font-body font-[450] rounded-lg transition-all ${tab===k ? "bg-[#e8ff4d] text-[#0b0f0e] font-[600]" : "text-[#6b7a6e] hover:text-white"}`}>
            {label}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Array.from({length:4}).map((_,i)=><div key={i} className="bg-[#0f1410] border border-white/[0.06] rounded-2xl h-[160px] animate-pulse" />)}
        </div>
      ) : list.length === 0 ? (
        <div className="bg-[#0f1410] border border-white/[0.06] rounded-2xl p-12 text-center">
          <Building2 className="w-10 h-10 text-[#3a4c3e] mx-auto mb-3" />
          <p className="text-[15px] text-white font-body font-[450] mb-1">No {tab === "vendors" ? "vendors" : "subcontractors"} yet</p>
          <p className="text-[13px] text-[#4a5c4e] font-body mb-4">Add your first entry to track compliance and contacts</p>
          <button onClick={() => tab === "vendors" ? setShowModal(true) : setShowSubModal(true)}
...
function VendorCard({ vendor: v }) {
  ...
  return (
    <div className="bg-[#0f1410] border border-white/[0.06] hover:border-white/[0.12] rounded-2xl p-5 transition-all">
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="min-w-0">
          <h3 className="font-body font-[500] text-[15px] text-white truncate">{v.name}</h3>
          <p className="text-[12px] text-[#4a5c4e] font-body mt-0.5">{v.trade}</p>
        </div>
      ...
      {v.rating > 0 && (
        <div className="flex items-center gap-0.5 mb-3">
          {Array.from({length: 5}).map((_, i) => (
            <Star key={i} className={`w-3 h-3 ${i < v.rating ? "text-[#e8ff4d]" : "text-white/10"}`} fill={i < v.rating ? "currentColor" : "none"} />
          ))}
        </div>
      )}
      <div className="space-y-1.5">
        {v.contact_name && <p className="text-[12px] text-[#6b7a6e] font-body">{v.contact_name}</p>}
        {v.phone && <div className="flex items-center gap-2 text-[12px] text-[#4a5c4e] font-body"><Phone className="w-3 h-3" />{v.phone}</div>}
        {v.email && <div className="flex items-center gap-2 text-[12px] text-[#4a5c4e] font-body"><Mail className="w-3 h-3" />{v.email}</div>}
        {v.insurance_expiry && <p className="text-[11px] text-[#3a4c3e] font-body">Insurance exp: {v.insurance_expiry}</p>}
      </div>
    </div>
  );
}
      ...
      <div className="bg-[#0f1410] border border-white/[0.08] rounded-2xl w-full max-w-[480px] my-4">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-body font-[500] text-[#8a9b8e] mb-1.5 tracking-wide uppercase">Insurance Expiry</label>
            ...
            <div>
              <label className="block text-[11px] font-body font-[500] text-[#8a9b8e] mb-1.5 tracking-wide uppercase">Compliance</label>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 bg-white/[0.04] border border-white/[0.08] text-[#8a9b8e] hover:text-white text-[14px] font-body py-3 rounded-xl transition-all">Cancel</button>
            <button type="submit" disabled={loading} className="flex-1 bg-[#e8ff4d] hover:bg-white text-[#0b0f0e] text-[14px] font-display font-[700] py-3 rounded-xl transition-all disabled:opacity-50">{loading ? "Adding…" : "Add Vendor"}</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 overflow-y-auto" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-[#0f1410] border border-white/[0.08] rounded-2xl w-full max-w-[480px] my-4">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-[11px] font-body font-[500] text-[#8a9b8e] mb-1.5 tracking-wide uppercase">Parent Vendor (optional)</label>
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" checked={form.w9_on_file} onChange={e => setForm(f => ({ ...f, w9_on_file: e.target.checked }))}
              className="w-4 h-4 rounded border border-white/20 bg-white/[0.04] accent-[#e8ff4d]" />
            <span className="text-[13px] text-[#8a9b8e] font-body">W-9 on file</span>
          </label>
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 bg-white/[0.04] border border-white/[0.08] text-[#8a9b8e] hover:text-white text-[14px] font-body py-3 rounded-xl transition-all">Cancel</button>
            <button type="submit" disabled={loading} className="flex-1 bg-[#e8ff4d] hover:bg-white text-[#0b0f0e] text-[14px] font-display font-[700] py-3 rounded-xl transition-all disabled:opacity-50">{loading ? "Adding…" : "Add Subcontractor"}</button>
          </div>
        </form>
      </div>
    </div>
