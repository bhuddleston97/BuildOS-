...
export default function AppProjectWizard() {
  ...
  async function handleCompanyNext() {
    ...
    try {
      ...
      setError(organizationError?.message || "Unable to create your company. Please try again.");
    } finally {
  ...
  async function handleFinish() {
    ...
    try {
      const { data: proj, error: bundleError } = await supabase.rpc("create_project_bundle", {
        project_data: {
  ...
  return (
    <div className="min-h-screen bg-[#09090b] flex items-start justify-center p-6 lg:p-10">
      <div className="w-full max-w-[760px]">

        {/* Header */}
        <div className="mb-8">
          <button onClick={() => navigate("/app/projects")}
            className="flex items-center gap-2 text-[#94a3b8] hover:text-white transition-colors text-[13px] font-body mb-6">
            <ArrowLeft className="w-4 h-4" /> Back to Projects
          </button>
          <h1 className="font-display font-[750] text-[28px] text-white tracking-[-0.03em] mb-1">
            New Project Wizard
          </h1>
          <p className="text-[14px] text-[#94a3b8] font-body">
              ...
              <div key={s.id} className="flex items-center flex-1">
                <div className="flex flex-col items-center">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-300 border ${
                    done ? "bg-[#60a5fa] border-[#60a5fa] text-[#09090b]"
                    : active ? "bg-[#60a5fa]/10 border-[#60a5fa]/40 text-[#60a5fa]"
                    : "bg-white/[0.03] border-white/[0.06] text-[#94a3b8]"
                  }`}>
                    {done ? <Check className="w-4 h-4" /> : <Icon className="w-4 h-4" />}
                  </div>
                  <span className={`text-[11px] font-body mt-1.5 ${active ? "text-[#60a5fa]" : done ? "text-white" : "text-[#94a3b8]"}`}>
                    {s.label}
                  </span>
                </div>
                {idx < STEPS.length - 1 && (
                  <div className={`flex-1 h-px mx-3 mb-5 transition-all duration-300 ${step > s.id ? "bg-[#60a5fa]/40" : "bg-white/[0.06]"}`} />
                )}
              </div>
            );
          })}
        ...
        {/* ── STEP 1: Company ── */}
        {step === 1 && (
          <div className="bg-[#18181b] border border-white/[0.06] rounded-2xl p-7 lg:p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-[13px] text-[#94a3b8] font-body">This is how your business will appear across the platform.</p>
              </div>
            </div>

            <div className="space-y-5">
              <div>
                <label className="block text-[11px] font-body font-[500] text-[#cbd5e1] mb-2 tracking-wide uppercase">Company Name *</label>
                <input
                  type="text" value={orgName} onChange={e => setOrgName(e.target.value)}
                  placeholder="e.g. Meridian Construction Group"
                  autoFocus
                  className="w-full bg-white/[0.04] border border-white/[0.08] focus:border-[#60a5fa]/50 text-white text-[16px] font-body px-4 py-3 rounded-xl outline-hidden transition-all placeholder:text-slate-500"
                />
                <p className="text-[12px] text-[#94a3b8] font-body mt-1.5">Enter the name of your construction company or organization.</p>
              </div>
              <div className="bg-[#60a5fa]/5 border border-[#60a5fa]/10 rounded-xl p-4">
                <p className="text-[13px] text-[#cbd5e1] font-body leading-relaxed">
            ...
            <div className="flex justify-end mt-8">
              <button
                ...
                className="flex items-center gap-2 bg-[#60a5fa] text-[#09090b] font-display font-[700] text-[14px] px-6 py-3 rounded-xl hover:bg-white transition-all disabled:opacity-50">
                {creatingCompany ? "Creating company…" : "Continue"} <ArrowRight className="w-4 h-4" />
              </button>
            </div>
          </div>
        )}
          ...
          <div className="bg-[#18181b] border border-white/[0.06] rounded-2xl p-7 lg:p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-[13px] text-[#94a3b8] font-body">Tell us about the project you're creating for <span className="text-[#60a5fa]">{orgName}</span>.</p>
              </div>
            </div>

            <div className="space-y-4">
              <div>
                <label className="block text-[11px] font-body font-[500] text-[#cbd5e1] mb-1.5 tracking-wide uppercase">Project Name *</label>
                <input type="text" value={project.name} onChange={setP("name")}
                  placeholder="e.g. Downtown Mixed-Use Tower"
                  autoFocus
                  className="w-full bg-white/[0.04] border border-white/[0.08] focus:border-[#60a5fa]/50 text-white text-[15px] font-body px-4 py-3 rounded-xl outline-hidden transition-all placeholder:text-slate-500" />
              </div>

              <div className="grid grid-cols-1 grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-body font-[500] text-[#cbd5e1] mb-1.5 tracking-wide uppercase">Client</label>
                ...
                <div>
                  <label className="block text-[11px] font-body font-[500] text-[#cbd5e1] mb-1.5 tracking-wide uppercase">Location</label>
                  <input type="text" value={project.location} onChange={setP("location")}
                    placeholder="City, State"
                    className="w-full bg-white/[0.04] border border-white/[0.08] focus:border-[#60a5fa]/50 text-white text-[14px] font-body px-3.5 py-2.5 rounded-xl outline-hidden transition-all placeholder:text-slate-500" />
                </div>
                <div>
                  <label className="block text-[11px] font-body font-[500] text-[#cbd5e1] mb-1.5 tracking-wide uppercase">Current Phase</label>
                ...
                <div>
                  <label className="block text-[11px] font-body font-[500] text-[#cbd5e1] mb-1.5 tracking-wide uppercase">Budget ($)</label>
                    ...
                    <button key={value} type="button" onClick={() => setProject(p => ({ ...p, status: value }))}
                      className={`px-4 py-2 rounded-xl text-[13px] font-body font-[450] border transition-all ${
                        project.status === value
                          ...
                          : "bg-white/[0.03] border-white/[0.06] text-[#94a3b8] hover:text-white"
                      }`}>
                      {label}
                    </button>
                  ))}
            ...
            <div className="flex items-center justify-between mt-8">
              <button onClick={() => { setError(""); setStep(1); }}
                className="flex items-center gap-2 text-[#94a3b8] hover:text-white transition-colors text-[14px] font-body">
                <ArrowLeft className="w-4 h-4" /> Back
              </button>
              <button onClick={handleStep2Next}
                className="flex items-center gap-2 bg-[#60a5fa] text-[#09090b] font-display font-[700] text-[14px] px-6 py-3 rounded-xl hover:bg-white transition-all">
        ...
        {step === 3 && (
          <div className="bg-[#18181b] border border-white/[0.06] rounded-2xl p-7 lg:p-8">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 rounded-xl bg-[#60a5fa]/10 flex items-center justify-center">
                <ListChecks className="w-5 h-5 text-[#60a5fa]" />
              </div>
              <div>
                <h2 className="font-display font-[650] text-[18px] text-white tracking-[-0.02em]">Create & Assign Tasks</h2>
                <p className="text-[13px] text-[#94a3b8] font-body">Add tasks for <span className="text-white">{project.name}</span> and assign them to your team.</p>
              </div>
            </div>

            <p className="text-[12px] text-[#94a3b8] font-body mb-6 ml-13">
              Each assigned team member will receive a notification to get started.
            </p>

            <div className="space-y-3 mb-4">
              {tasks.map((t, i) => (
                <div key={i} className="bg-[#09090b] border border-white/[0.06] rounded-xl p-4 group">
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded-full bg-[#60a5fa]/10 text-[#60a5fa] text-[11px] font-display font-[700] flex items-center justify-center shrink-0 mt-0.5">
                      {i + 1}
                    </div>
                    <div className="flex-1 space-y-3">
                      <input
                        ...
                        className="w-full bg-white/[0.04] border border-white/[0.06] focus:border-[#60a5fa]/40 text-white text-[14px] font-body px-3.5 py-2.5 rounded-xl outline-hidden transition-all placeholder:text-slate-500"
                      />
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div>
                          <label className="block text-[10px] font-body font-[500] text-[#94a3b8] mb-1 tracking-wide uppercase">Assign To</label>
                          <select
                            value={t.assigned_user_id}
                            onChange={e => handleMemberSelect(i, e.target.value)}
                            className="w-full bg-white/[0.04] border border-white/[0.06] focus:border-[#60a5fa]/40 text-white text-[13px] font-body px-3 py-2 rounded-lg outline-hidden transition-all">
                        ...
                        <div>
                          <label className="block text-[10px] font-body font-[500] text-[#94a3b8] mb-1 tracking-wide uppercase">Priority</label>
                          <select value={t.priority} onChange={e => setTask(i, "priority", e.target.value)}
                            className="w-full bg-white/[0.04] border border-white/[0.06] focus:border-[#60a5fa]/40 text-white text-[13px] font-body px-3 py-2 rounded-lg outline-hidden">
                        ...
                        <div>
                          <label className="block text-[10px] font-body font-[500] text-[#94a3b8] mb-1 tracking-wide uppercase">Due Date</label>
                    ...
                    {tasks.length > 1 && (
                      <button onClick={() => removeTask(i)}
                        className="text-[#94a3b8] hover:text-rose-400 transition-colors mt-0.5 p-1 opacity-0 group-hover:opacity-100">
                        <X className="w-4 h-4" />
                      </button>
                    )}
                  </div>
            ...
            <button onClick={addTask}
              className="flex items-center gap-2 text-[#94a3b8] hover:text-[#60a5fa] transition-colors text-[13px] font-body border border-dashed border-white/[0.08] hover:border-[#60a5fa]/20 rounded-xl px-4 py-2.5 w-full justify-center">
            ...
            <div className="flex items-center justify-between mt-8">
              ...
              <button onClick={handleFinish} disabled={saving}
                className="flex items-center gap-2 bg-[#60a5fa] text-[#09090b] font-display font-[700] text-[14px] px-6 py-3 rounded-xl hover:bg-white transition-all disabled:opacity-60">
        ...
        {step === 4 && createdProject && (
          <div className="bg-[#18181b] border border-white/[0.06] rounded-2xl p-8 lg:p-10 text-center">
            <div className="w-16 h-16 rounded-2xl bg-[#60a5fa]/10 flex items-center justify-center mx-auto mb-5">
            ...
            <p className="text-[15px] text-[#94a3b8] font-body mb-2">
              Your project has been created for <span className="text-white">{orgName}</span>.
            </p>
            <p className="text-[13px] text-[#94a3b8] font-body mb-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <button onClick={() => navigate("/app/tasks")}
                className="flex items-center gap-2 justify-center bg-[#60a5fa] text-[#09090b] font-display font-[700] text-[14px] px-6 py-3 rounded-xl hover:bg-white transition-all">
              ...
              <button onClick={() => { setStep(1); setOrgName(""); setProject({ name:"",client:"",location:"",phase:"Planning",budget:"",status:"active",description:"" }); setTasks([{title:"",assigned_to:"",assigned_user_id:"",priority:"medium",due_date:"",description:""}]); setCreatedProject(null); }}
                className="flex items-center gap-2 justify-center text-[#94a3b8] hover:text-white font-body text-[14px] px-6 py-3 rounded-xl transition-all">
