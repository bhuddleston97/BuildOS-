...
export default function AppReports() {
  ...
  useEffect(() => {
    ...
    listRows("field_reports", { perPage: 50, sort: "-created", signal: ctrl.signal })
      .then(setReports)
    ...
    <div className="p-6 lg:p-8 max-w-[1200px] mx-auto">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-[14px] text-[#6b7a6e] font-body mt-0.5">{reports.length} report{reports.length !== 1 ? "s" : ""} submitted</p>
        </div>
      ...
      {loading ? (
        <div className="space-y-3">
          {Array.from({length: 4}).map((_, i) => <div key={i} className="bg-[#0f1410] border border-white/[0.06] rounded-2xl h-[120px] animate-pulse" />)}
        </div>
      ) : reports.length === 0 ? (
        <div className="bg-[#0f1410] border border-white/[0.06] rounded-2xl p-12 text-center">
          <FileText className="w-10 h-10 text-[#3a4c3e] mx-auto mb-3" />
          <p className="text-[15px] text-white font-body font-[450] mb-1">No field reports yet</p>
          <p className="text-[13px] text-[#4a5c4e] font-body mb-4">Submit your first daily report from the field</p>
          <button onClick={() => setShowModal(true)}
        ...
        <div className="space-y-3">
          {reports.map(r => (
            <div key={r.id} className="bg-[#0f1410] border border-white/[0.06] rounded-2xl p-5">
              <div className="flex items-start justify-between gap-4 mb-3">
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    {r.report_date && <span className="text-[12px] text-[#4a5c4e] font-body">{r.report_date}</span>}
                  </div>
                </div>
                <div className="flex items-center gap-4 shrink-0 text-[12px] text-[#4a5c4e] font-body">
              ...
              {r.work_performed && <p className="text-[14px] text-[#8a9b8e] font-body leading-relaxed">{r.work_performed}</p>}
...
function NewReportModal({ onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 overflow-y-auto" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-[#0f1410] border border-white/[0.08] rounded-2xl w-full max-w-[500px] my-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              ...
              <select value={form.project_id} onChange={set("project_id")}
                className="w-full bg-white/[0.04] border border-white/[0.08] focus:border-[#e8ff4d]/50 text-white text-[14px] font-body px-3.5 py-2.5 rounded-xl outline-hidden">
                <option value="">Select project…</option>
                {projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-[11px] font-body font-[500] text-[#8a9b8e] mb-1.5 tracking-wide uppercase">Submitted By</label>
            ...
            <div>
              <label className="block text-[11px] font-body font-[500] text-[#8a9b8e] mb-1.5 tracking-wide uppercase">Date</label>
              <input type="date" value={form.report_date} onChange={set("report_date")}
                className="w-full bg-white/[0.04] border border-white/[0.08] focus:border-[#e8ff4d]/50 text-white text-[14px] font-body px-3.5 py-2.5 rounded-xl outline-hidden" />
            </div>
            <div>
              <label className="block text-[11px] font-body font-[500] text-[#8a9b8e] mb-1.5 tracking-wide uppercase">Weather</label>
            ...
            <div>
              <label className="block text-[11px] font-body font-[500] text-[#8a9b8e] mb-1.5 tracking-wide uppercase">Crew Count</label>
              <input type="number" min="0" value={form.crew_count} onChange={set("crew_count")} placeholder="0"
                className="w-full bg-white/[0.04] border border-white/[0.08] focus:border-[#e8ff4d]/50 text-white text-[14px] font-body px-3.5 py-2.5 rounded-xl outline-hidden transition-all placeholder:text-[#3a4c3e]" />
            </div>
            <div>
              <label className="block text-[11px] font-body font-[500] text-[#8a9b8e] mb-1.5 tracking-wide uppercase">Hours Worked</label>
              <input type="number" min="0" value={form.hours_worked} onChange={set("hours_worked")} placeholder="0"
                className="w-full bg-white/[0.04] border border-white/[0.08] focus:border-[#e8ff4d]/50 text-white text-[14px] font-body px-3.5 py-2.5 rounded-xl outline-hidden transition-all placeholder:text-[#3a4c3e]" />
            </div>
            <div>
              <label className="block text-[11px] font-body font-[500] text-[#8a9b8e] mb-1.5 tracking-wide uppercase">Safety Incidents</label>
              <input type="number" min="0" value={form.safety_incidents} onChange={set("safety_incidents")} placeholder="0"
                className="w-full bg-white/[0.04] border border-white/[0.08] focus:border-[#e8ff4d]/50 text-white text-[14px] font-body px-3.5 py-2.5 rounded-xl outline-hidden transition-all placeholder:text-[#3a4c3e]" />
            </div>
          </div>
          <div>
            <label className="block text-[11px] font-body font-[500] text-[#8a9b8e] mb-1.5 tracking-wide uppercase">Work Performed *</label>
            <textarea value={form.work_performed} onChange={set("work_performed")} rows={3} placeholder="Describe what was accomplished today…"
              className="w-full bg-white/[0.04] border border-white/[0.08] focus:border-[#e8ff4d]/50 text-white text-[14px] font-body px-3.5 py-2.5 rounded-xl outline-hidden transition-all resize-none placeholder:text-[#3a4c3e]" />
          </div>
          <div>
            <label className="block text-[11px] font-body font-[500] text-[#8a9b8e] mb-1.5 tracking-wide uppercase">Issues / Notes</label>
