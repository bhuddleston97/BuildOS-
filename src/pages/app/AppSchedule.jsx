...
export default function AppSchedule() {
  const { rows: items, loading, setRows: setItems } = useRealtimeRows("schedule_items", { perPage: 100, sort: "planned_start" });
    ...
    <div className="p-6 lg:p-8 max-w-[1200px] mx-auto">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-[14px] text-[#94a3b8] font-body mt-0.5">{items.length} milestone{items.length !== 1 ? "s" : ""} scheduled</p>
        </div>
      ...
      {loading ? (
        <div className="space-y-3">
          {Array.from({length: 5}).map((_, i) => <div key={i} className="bg-[#18181b] border border-white/[0.06] rounded-2xl h-[72px] animate-pulse" />)}
        </div>
      ) : items.length === 0 ? (
        <div className="bg-[#18181b] border border-white/[0.06] rounded-2xl p-12 text-center">
          ...
          <p className="text-[13px] text-[#94a3b8] font-body mb-4">Add key dates and milestones to keep projects on track</p>
          <button onClick={() => setShowModal(true)}
        ...
        <div className="space-y-3">
          {items.map(item => {
            ...
            return (
              <div key={item.id} className="bg-[#18181b] border border-white/[0.06] rounded-2xl px-5 py-4 flex items-center gap-4">
                <div className={`w-2 h-2 rounded-full shrink-0 ${item.status === "completed" ? "bg-emerald-400" : isLate ? "bg-rose-400" : "bg-[#60a5fa]"}`} />
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] text-white font-body font-[450]">{item.title || item.name || "Untitled"}</p>
                  {item.description && <p className="text-[12px] text-[#94a3b8] font-body mt-0.5 truncate">{item.description}</p>}
                </div>
                <div className="text-right shrink-0">
                  {planned && <p className="text-[13px] text-[#cbd5e1] font-body">{planned}</p>}
...
function NewMilestoneModal({ onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-[#18181b] border border-white/[0.08] rounded-2xl w-full max-w-[440px]">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-[11px] font-body font-[500] text-[#cbd5e1] mb-1.5 tracking-wide uppercase">Milestone Title *</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-body font-[500] text-[#cbd5e1] mb-1.5 tracking-wide uppercase">Start Date</label>
              <input type="date" value={form.planned_start} onChange={set("planned_start")}
                className="w-full bg-white/[0.04] border border-white/[0.08] text-white text-[14px] font-body px-3.5 py-2.5 rounded-xl outline-hidden" />
            </div>
            <div>
              <label className="block text-[11px] font-body font-[500] text-[#cbd5e1] mb-1.5 tracking-wide uppercase">End Date</label>
