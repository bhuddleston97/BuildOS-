...
const statusCfg = {
  pending:   { label: "Pending",   color: "text-[#cbd5e1]", bg: "bg-white/[0.04]",    border: "border-white/[0.08]",    Icon: Clock },
...
export default function AppChangeOrders() {
  const { rows: orders, loading, setRows: setOrders } = useRealtimeRows("change_orders", { perPage: 100, sort: "-created" });
    ...
    <div className="p-6 lg:p-8 max-w-[1200px] mx-auto">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-[14px] text-[#94a3b8] font-body mt-0.5">{orders.length} total · {orders.filter(o=>o.status==="pending"||o.status==="in_review").length} awaiting action</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <div className="bg-[#18181b] border border-white/[0.06] rounded-2xl p-5">
          <p className="text-[12px] text-[#94a3b8] font-body mb-2">Approved cost impact</p>
          <p className="font-display font-[700] text-[22px] text-emerald-400 tracking-[-0.02em]">+{fmt(totalApproved)}</p>
        </div>
        <div className="bg-[#18181b] border border-white/[0.06] rounded-2xl p-5">
          <p className="text-[12px] text-[#94a3b8] font-body mb-2">Pending cost exposure</p>
      ...
      <div className="flex items-center gap-2 mb-5 flex-wrap">
        {[["all","All"],["pending","Pending"],["in_review","In Review"],["approved","Approved"],["rejected","Rejected"]].map(([k, label]) => (
          <button key={k} onClick={() => setFilter(k)}
            ...
            {label} <span className={`ml-1 text-[11px] ${filter === k ? "text-[#60a5fa]/60" : "text-[#94a3b8]"}`}>{counts[k]}</span>
          </button>
        ))}
      </div>

      {loading ? (
        <div className="space-y-3">{Array.from({length:4}).map((_,i)=><div key={i} className="bg-[#18181b] border border-white/[0.06] rounded-2xl h-[100px] animate-pulse" />)}</div>
      ) : filtered.length === 0 ? (
        <div className="bg-[#18181b] border border-white/[0.06] rounded-2xl p-12 text-center">
          <FileEdit className="w-10 h-10 text-[#94a3b8] mx-auto mb-3" />
          <p className="text-[15px] text-white font-body font-[450] mb-1">No change orders</p>
          <p className="text-[13px] text-[#94a3b8] font-body mb-4">Document scope changes and cost impacts here</p>
          <button onClick={() => setShowModal(true)}
        ...
        <div className="space-y-3">
          {filtered.map(o => {
            ...
            return (
              <div key={o.id} className="bg-[#18181b] border border-white/[0.06] rounded-2xl p-5">
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div className="min-w-0">
                    <h3 className="font-body font-[500] text-[15px] text-white truncate">{o.title}</h3>
                    <p className="text-[12px] text-[#94a3b8] font-body mt-0.5">{projectName(o.project_id)} · {o.requested_by && `Requested by ${o.requested_by}`}</p>
                  </div>
                ...
                {o.description && <p className="text-[13px] text-[#94a3b8] font-body leading-relaxed mb-3">{o.description}</p>}
                <div className="flex items-center justify-between flex-wrap gap-3">
                  <div className="flex items-center gap-5">
                    <div>
                      <p className="text-[10px] text-[#94a3b8] font-body uppercase tracking-wide">Cost Impact</p>
                      <p className="text-[14px] font-display font-[600] text-white">+{fmt(o.cost_impact || 0)}</p>
                    </div>
                    {o.schedule_impact_days > 0 && (
                      <div>
                        <p className="text-[10px] text-[#94a3b8] font-body uppercase tracking-wide">Schedule</p>
    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 overflow-y-auto" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-[#18181b] border border-white/[0.08] rounded-2xl w-full max-w-[480px] my-4">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-[11px] font-body font-[500] text-[#cbd5e1] mb-1.5 tracking-wide uppercase">Description</label>
            <textarea value={form.description} onChange={set("description")} rows={3} placeholder="Describe the scope change and reason…"
              className="w-full bg-white/[0.04] border border-white/[0.08] focus:border-[#60a5fa]/50 text-white text-[14px] font-body px-3.5 py-2.5 rounded-xl outline-hidden resize-none transition-all placeholder:text-slate-500" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-body font-[500] text-[#cbd5e1] mb-1.5 tracking-wide uppercase">Cost Impact ($)</label>
              <input type="number" value={form.cost_impact} onChange={set("cost_impact")} placeholder="42000"
                className="w-full bg-white/[0.04] border border-white/[0.08] focus:border-[#60a5fa]/50 text-white text-[14px] font-body px-3.5 py-2.5 rounded-xl outline-hidden transition-all placeholder:text-slate-500" />
            </div>
            <div>
              <label className="block text-[11px] font-body font-[500] text-[#cbd5e1] mb-1.5 tracking-wide uppercase">Schedule (days)</label>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose}
              className="flex-1 bg-white/[0.04] border border-white/[0.08] text-[#cbd5e1] hover:text-white text-[14px] font-body py-3 rounded-xl transition-all">Cancel</button>
