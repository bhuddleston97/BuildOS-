...
const typeCfg = {
  ...
  task_due:        { label: "Task",          Icon: CheckSquare,  color: "text-[#8a9b8e]",  bg: "bg-white/[0.06]" },
  ...
  system:          { label: "System",        Icon: Info,         color: "text-[#6b7a6e]",  bg: "bg-white/[0.04]" },
  ai_insight:      { label: "AI Insight",    Icon: Zap,          color: "text-[#e8ff4d]",  bg: "bg-[#e8ff4d]/10" },
};
...
export default function AppNotifications() {
  ...
  useEffect(() => {
    ...
    listRows("notifications", { perPage: 100, sort: "-created", signal: ctrl.signal })
      .then(setNotifs)
    ...
    <div className="p-6 lg:p-8 max-w-[900px] mx-auto">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-[14px] text-[#6b7a6e] font-body mt-0.5">
            {unreadCount > 0 ? `${unreadCount} unread` : "All caught up"}
          </p>
        </div>
        {unreadCount > 0 && (
          <button onClick={markAllRead}
            className="flex items-center gap-2 text-[13px] text-[#6b7a6e] hover:text-[#e8ff4d] border border-white/[0.08] hover:border-[#e8ff4d]/30 px-4 py-2 rounded-xl transition-all font-body">
        ...
        {[
          ...
          <button key={k} onClick={() => setFilter(k)}
            className={`text-[12.5px] font-body font-[450] px-3 py-1.5 rounded-lg transition-all border ${
              filter === k
                ? "bg-[#e8ff4d]/10 text-[#e8ff4d] border-[#e8ff4d]/20"
                : "text-[#6b7a6e] hover:text-white hover:bg-white/[0.04] border-transparent"
            }`}>
            {label}
          </button>
        ))}
      ...
      {loading ? (
        <div className="space-y-3">{Array.from({length:5}).map((_,i)=><div key={i} className="bg-[#0f1410] border border-white/[0.06] rounded-2xl h-[80px] animate-pulse" />)}</div>
      ) : filtered.length === 0 ? (
        <div className="bg-[#0f1410] border border-white/[0.06] rounded-2xl p-12 text-center">
          <Bell className="w-10 h-10 text-[#3a4c3e] mx-auto mb-3" />
          <p className="text-[15px] text-white font-body font-[450] mb-1">No notifications</p>
          <p className="text-[13px] text-[#4a5c4e] font-body">You're all caught up — alerts will appear here as your projects progress</p>
        </div>
      ) : (
        <div className="space-y-2">
          {filtered.map(n => {
            ...
            return (
              <div key={n.id}
                className={`bg-[#0f1410] border border-white/[0.06] border-l-4 ${border} rounded-2xl px-5 py-4 flex items-start gap-4 transition-all ${!n.read ? "bg-white/[0.02]" : "opacity-70"}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className={`text-[14px] font-body font-[${n.read ? "450" : "500"}] ${n.read ? "text-[#8a9b8e]" : "text-white"}`}>{n.title}</p>
                      <p className="text-[13px] text-[#4a5c4e] font-body mt-0.5 leading-relaxed">{n.message}</p>
                    </div>
                    {!n.read && (
                      <button onClick={() => markRead(n.id)}
                        className="shrink-0 text-[#4a5c4e] hover:text-[#e8ff4d] transition-colors p-1.5 rounded-lg hover:bg-white/[0.04]"
                        title="Mark as read">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    {!n.read && <span className="w-1.5 h-1.5 bg-[#e8ff4d] rounded-full" />}
                  </div>
                </div>
              </div>
            );
